...
# Or you can go to Session -> Set WD
```
...
```{r}
relig_income_long<- pivot_longer(relig_income, cols = c(colnames(relig_income[2:11])), names_to= "income", values_to= "totalParticipants")
```
...
```{r}
longBillboard<- pivot_longer(billboard, cols = 4:79, names_to = "week", values_to = "score")
#find column positions
colnames(billboard)
```
#### pipes!
```{r}
longBill<- billboard %>% pivot_longer(cols=4:79, names_to = "week", values_to = "score")

billboard %>% 
  pivot_longer(cols=4:79, names_to = "week", values_to = "score") %>%
  summarize(mean=mean(score, na.rm=TRUE)) #na.rm=TRUE ignores NA entries in analysis
```
...
```{r}
aggressionWide<- pivot_wider(aggressionLongDat, id_cols = 1:4, names_from = "timePoint", values_from = "aggressionScore")
```

```{r}
aggressionLongDat %>% pivot_wider(id_cols=1:4, names_from = "timePoint", values_from = "aggressionScore")
```
...
```{r}
aggressionWideDat2<- spread(aggressionLongDat, key = "timePoint", value = "aggressionScore")
```
...
# goes with melt!!! Yucky!!! Disgostang!!!
# OH MY GOD THAT *IS* GROSS I HATE THIS
...
```{r}
wideChick<- pivot_wider(ChickWeight, id_cols = c("Chick", "Diet"), names_from = "Time", values_from = "weight")
# pivot_wider(ChickWeight, id_cols = 1:2, names_from = "Time", values_from = "weight")
# This works because only Chick and Diet columns are left! When Time and weight are taken out, R only looks at whatever columns are remaining
```

3. Convert from wide to long. 
```{r}
longChick<- pivot_longer(wideChick, cols = 3:14, names_to = "Time", values_to = "weight")

longChick2<- na.omit(longChick) #BE SO SO SO CAREFUL WITH THIS
```
...
```{r}
wideSleep<- pivot_wider(sleep, id_cols = 1, names_from = "group", values_from = "extra")
```

4) Go back to the chick weight dataset and take all data from chick 3 and put it in wide format 
```{r}
wideChickThree<- ChickWeight %>% filter(Chick==3) %>% pivot_wider(id_cols = c("Chick", "Diet"), names_from = "Time", values_from = "weight")
```
...
```{r}
#Hamilton anxiety rating scale(<17 mild, 18-24 mild/moderate, 25-30 moderate/severe, 30-56 severe)
simulatedAnxietyScore<- sample(48, x= 0:56, replace = TRUE)
...
`right_join()` does the same thing, but starts on the right. 
...
```{r}
execFunctionSimulated<- sample(48, x= 0:16, replace=TRUE)
execFunctionSimulated

execFunctionDat<- data.frame(subjNo=aggressionDat$subjNo, execFunctionSimulated)
```

2. Join your dataset in #1 to the original Aggression dataset. 
```{r}
execAggressionDat<- left_join(aggressionDat, execFunctionDat)
```
...
aggressionDat %>% select(parentReportGender, parentReportGenderThree)
...
#### Practice: 
1. Create a column in the aggression dataset that contains condition as a factor. 
```{r}
aggressionDat$conditionTwo<- as.factor(aggressionDat$condition)
is.factor(aggressionDat$conditionTwo)
```

2. Change the levels to "e" for exclusion and "i" for inclusion. 
```{r}
levels(aggressionDat$conditionTwo)<- c("e", "i")
```

3. Check that you have done this successfully. 
```{r}
aggressionDat %>% select(condition, conditionTwo) #yayyyyyyyyy
```
...
```{r}
aggressionDat$conditionTwo<- relevel(aggressionDat$conditionTwo, ref="i")
aggressionDat$conditionTwo
```
...
```{r}
separate(aggressionDat, subjNo, into=c("conditionMarker", "subjectNo"), sep=1) #this is in tidyverse
```
...
```{r}
updateStarWars<- starwars %>% mutate(meters= height/100)
updateStarWars
```
