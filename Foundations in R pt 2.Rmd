...
```{r}
"is.classname"
is.factor(mtcars$`automatic?`)
"as.classname"
#make a new column 
mtcars$`automatic?`<- as.factor(mtcars$`automatic?`)
mtcars$`automatic?`
class(mtcars$`automatic?`)
...
```{r}
mtcars$automatic2 <- as.numeric(mtcars$`automatic?`)
mtcars$automatic2

mtcars %>% select(`automatic?`, automatic2) #compare before saving

mtcars$automatic3<- as.numeric(levels(mtcars$`automatic?`)[mtcars$`automatic?`])
mtcars %>% select(`automatic?`, automatic3) #compare before saving
```
...
summary(mtcars$hp)
summary(mtcars$hp2)
# numeric data displays max, min, median, and quartiles. Factor data shows a string of numbers with frequencies
```
...
```{r}
addTwo<- function(number){number + 2}
```
...
```{r}
multiplySeven<- function(number){number * 7}
```
test it out
```{r}
multiplySeven(5)
...
`paste()`
```{r}
myFavorite<- function(thing){paste("My favorite food is", thing)}
```
test
```{r}
...
numberDat$multiply<- multiplyBy(oneToTen, tenToOne)
```
...
```{r}
divide100<- function(number){number/100}
```

2. Test your function on the mtcars mpg column.
```{r}
divide100(mtcars$mpg)
```

3. Make a function that finds the mean of a column and divides it by the standard deviation of another column.
```{r}
meanDivideBySD<- function(thing1, thing2){mean(thing1)/sd(thing2)}
```

4. Test it on the mpg column and hp column. 
```{r}
meanDivideBySD(mtcars$mpg, mtcars$hp)
```
```{r}
thisNonsense<- function(number1, number2, number3){((number1)-(number2))/sd(number3)}
thisNonsense(mtcars$mpg, mtcars$cyl, mtcars$disp)
```
...
ChickWeight$beginEnd[ChickWeight$Time<5]<- "beginning"
```
...
```{r}
for(i in 1:nrow(ChickWeight)){ChickWeight$beginEnd[i]<-ChickWeight$Time[i]>10}
```
...
```{r}
tenToHundred<- 10:100
chart<- cbind.data.frame(tenToHundred)
chart
chart$backwards<- NA
for(i in 1:nrow(chart)){chart$backwards[i]<-chart$tenToHundred[i]*4}

vectNum<- 10:100
for(i in 1:length(vectNum)){vectNum[i]<-vectNum[i]*4}
```
...
for(i in 1:nrow(mtcars)){mtcars$goodMileage[i]<-mtcars$mpg[i]>25}
```
...
```{r}
data(ChickWeight)
ChickWeight$bigChick<- NA
for(i in 1:nrow(ChickWeight)){
  if(ChickWeight$weigh[i]>100){
    ChickWeight$bigChick[i]<- "large"
  }else{ChickWeight$bigChick[i]<- "small"}
}
```
for(){
  if(){
...
```{r}
mtcars$transmissiontype<- NA
mtcars$transmissionType<- ifelse(mtcars$am==1, "manual", "automatic")

mtcars$test<- ifelse(mtcars$gear==4, "a", ifelse(mtcars$gear==5, "b", "c"))
```

### Bonus for loop

based on the island the penguins are on, fill in a new column that tells us whose favotite it is
Tejiri = Dream
Sawyer = Biscoe
Rachel = Torgersen
```{r}
