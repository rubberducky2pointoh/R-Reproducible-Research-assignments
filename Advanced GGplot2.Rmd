...
First, we need some data. For today, we'll use simulated data:
```{r}
set.seed(137)
...
studentInterest<- sample(c("very low", "low", "moderate", "high", "very high"), 100, replace = TRUE)
```
...
library(tidyverse)
```
Predict # of boba shops based on school climate & student interest
...
```
```{r}
subset(bobaDat, bobaDat$schoolClimate=="cool" & bobaDat$studentInterest=="high")
```

use this output to create a graph 

```{r}
ggplot(summaryBoba, aes(schoolClimate, Mean, fill=studentInterest))+
  geom_bar(stat="identity", position=position_dodge())+
  geom_errorbar(aes(ymin = Mean-stderr, ymax = Mean+stderr), position = position_dodge(), width=.90)+
  theme_bw()
```

Alternatively, could use facet wrap with boxplots
```{r}
ggplot(bobaDat, aes(schoolClimate, boba))+
  geom_boxplot()+
  facet_wrap(~studentInterest)
```

Finally, could use ggeffects package. Need to load the library
```{r}
library(ggeffects) #why doesn't it like me
```
...
using `facet wrap()`
```{r}
ggplot(bobaDat, aes(studentSize, boba, color=studentInterest))+
  geom_point()+
