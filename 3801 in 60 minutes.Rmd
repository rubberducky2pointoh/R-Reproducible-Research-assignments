...
#t.test(group1,group2) <- only when there are two groups in different columns
```
...
```{r}
t.test(disp~ vs, data = mtcars, var.equal = TRUE, alternative = "less")
...
ggplot(smh, aes(stressLevel, creditLoad))+
  geom_point()+
  geom_smooth(method = "lm")
...
```{r}
mtcarslm2<- lm(mpg~ cyl * disp, data = mtcars)
summary(mtcarslm2)
```
...
```{r}
biserial.cor(goodBadSleep$anxietyScore, goodBadSleep$sleepQuality, level=2)
ggplot(goodBadSleep, aes(sleepQuality, anxietyScore))+
  geom_point()+
  geom_smooth(method="lm")
```
...
**chi squared tests** 
compare frequencies of nominal data. Goodness of fit test: do our data differ from the expected group?
Test of indep.: are our two variables related?
...
chisq.test(c(217, 319, 48), p= c(.34, .33, .33))
```
