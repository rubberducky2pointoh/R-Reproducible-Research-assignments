---
title: "Statistics Write up and workflow"
...
```{r numberOneOutcomeOne, echo=FALSE}
data("diamonds")
ggplot(diamonds, aes(carat, depth, color=cut))+
  geom_point()+
  theme_bw()
```
...
no code no output
```{r numberTwoOutcomeOne, include=FALSE}
ggplot(diamonds, aes(carat, depth, color=cut))+
  geom_point()+
  theme_bw()
```
...
If you want to actually run R code, you need to modify it slightly to `r "insert code here."` For instance, we could do "the mean of the mpg column is `r mean(mtcars$mpg, digits=2)`
...
The correlation between carat and cut is `r cor(diamonds$carat, as.numeric(diamonds$cut))`
