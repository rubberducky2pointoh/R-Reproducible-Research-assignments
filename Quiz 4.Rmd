...
```{r}
smh <- read.csv("~/PSY 4802/students_mental_health_survey fall 2025 (1).csv")
data(smh)
levels(as.factor(smh$sleepQuality))
t.test(stressLevel ~ sleepQuality, data = smh, var.equal = TRUE)
```
...
```{r}
view(sleep)
pivotlonger(sleep, id=c("ID")), names_from="group", values_from="extra")
```

13. Make a dataset of carat, cut, clarity and price
```{r}
diamondsSub<- subset(diamonds$carat, diamonds$cut, diamonds$clarity, diamonds$price)
```
...
```{r}
sw<-data(starwars)
sw_hm <- ggplot(sw, vals = c(height, mass)) + 
  geom_point()
...
```{r}
ggplot2(penguins, aes(species))+geom_bar(color="black", color="orange") + theme_minimal()+ xlab("species") ylab("frequency")+ ggtitle("Penguin Species")
```
...
```{r}
ggplot(mtcars, aes(mpg, fill = vs)) + geom_histogram() + scale_color_brewer(type = "qual", palette = "Set2")
```

19. Create a graph of cortisol level from the student mental health dataset. 
```{r}
ggplot(smh$cortisolLevel)+ geom_histogram + geom_vline(xintercept = mean(smh$cortisolLevel), color="orchid", size= 1)+ theme_classic()+ geom_vline(xintercept = median(smh$cortisolLevel), size= 1, linetype=dashed, color= "orchid4")+ geom_vline(xintercept = mfv(smh$cortisolLevel), color="wheat", size= 1)
```

20. find the means and sds of the bill_length_mm column and bill_depth_mm column in the penguins dataset. 
```{r}
mean(penguins$bill_length_mm, na.rm = TRUE)
sd(penguins$bill_lengh_mm, na.rm = TRUE) 
